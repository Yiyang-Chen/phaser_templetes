...
**AI生成的配置文件常见问题**：很多AI生成的 `audio-config.json` 文件只包含 `audioTypes` 部分，**缺少关键的 `assets` 定义部分**。这会导致音频无法正常播放！

完整的 `audio-config.json` 必须包含两个主要部分：

1. **`audioTypes`**: 定义播放逻辑和映射关系
2. **`assets`**: 定义具体的音频资源配置（**经常被遗漏**）
...
**步骤 2**: 在 `audio-config.json` 中配置播放逻辑
```json
{
  "audioTypes": {
    "bgm": {
...
**步骤 3**: 在 `audio-config.json` 中定义资源配置
```json
{
  "assets": {
    "bgm": {
      "boss_theme": {
        "url": "bgm_boss_battle",  // 引用 game_config.json 中的资源键
        "preload": false,
        "volume": 0.8,
        "loop": true
      }
...
**步骤 3**: 在 `audio-config.json` 中定义音效资源配置
```json
{
  "assets": {
    "sfx": {
      "enemy_roar": {
        "url": "sfx_enemy_roar",  // 引用 game_config.json 中的资源键
        "preload": true,
        "volume": 0.9
      }
    }
...
1. **🚨 配置完整性**: `audio-config.json` 必须同时包含 `audioTypes` 和 `assets` 两个部分，缺少任何一个都会导致音频无法播放
2. **资源键一致性**: `audio-config.json` 中的 `url` 必须与 `game_config.json` 中的 `key` 完全匹配
3. **AI生成配置检查**: AI生成的配置文件经常遗漏 `assets` 部分，使用前务必检查配置完整性
4. **用户交互**: 现代浏览器需要用户交互后才能播放音频，AudioManager 会自动处理
5. **多格式支持**: AudioLoader 自动处理多种音频格式，无需手动指定格式
6. **URL格式要求**: Phaser音频URL必须包含格式参数，AudioLoader会自动添加 `&format=.mp3` 等参数
7. **缓存优化**: 相同URL的音频只会加载一次，后续使用别名系统节省内存
8. **预加载策略**: 合理配置 `preload` 以平衡加载时间和内存使用
