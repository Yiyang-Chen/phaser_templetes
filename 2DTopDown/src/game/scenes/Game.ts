...
export class Game extends Scene
{
    ...
    bullets: Bullet[] = [];  // 改为普通数组，Bullet自己管理physics
    triggers: Trigger[];
    ...
    create ()
    {
        ...
        // Bullet已经在构造函数中自己添加了physics body
        // 这里只需要加入列表并设置碰撞检测
        eventBus.on(GameEvent.BULLET_CREATED, (data) => {
            if (data.bullet) {
                console.log(`[Game] BULLET_CREATED event received, bullet at (${data.bullet.x}, ${data.bullet.y})`);
                this.bullets.push(data.bullet);
                console.log(`[Game] Bullets array length: ${this.bullets.length}`);
                // 为这个子弹设置碰撞检测
                this.setupBulletCollisions(data.bullet);
                console.log(`[Game] Bullet collisions setup complete`);
            }
        });
    ...
    private createOverleapEvents() {
        ...
        // Bullet collisions are now set up individually in setupBulletCollisions()
        // when each bullet is created via BULLET_CREATED event
     ...
     * 为单个子弹设置所有必要的碰撞检测
     */
    private setupBulletCollisions(bullet: Bullet) {
        console.log(`[Game] setupBulletCollisions() called, bullet active: ${bullet.active}, body exists: ${!!bullet.body}`);
        ...
        // 子弹 vs 地图层
        this.layers.forEach(layer => {
            console.log(`[Game] Setting up collider between bullet and layer:`, layer.name || 'unnamed');
    ...
    private handleBulletEnemyCollision(bullet: any, enemy: any) {
        ...
        // 获取当前伤害值（考虑穿透衰减）
        const damage = bulletInstance.hitEnemy();
    ...
    update() {
        // ⚠️ 重要：手动更新所有子弹（因为用的是普通数组，不是Group）
        if (this.bullets) {
            this.bullets = this.bullets.filter(bullet => {
                if (bullet.active) {
                    bullet.update();  // 调用子弹的update方法
                    return true;
                }
                return false;  // 移除不活跃的子弹
            });
        }
