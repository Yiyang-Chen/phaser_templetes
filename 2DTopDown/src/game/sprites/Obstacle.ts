...
export class Obstacle extends Phaser.Physics.Arcade.Sprite {
    ...
    constructor(scene: Scene, obstacleObject: Phaser.Types.Tilemaps.TiledObject) {
        ...
        if (this.isMovable) {
            // Dynamic body for movable obstacles (TopDown)
            scene.physics.add.existing(this, false);
            const body = this.body as Phaser.Physics.Arcade.Body;
            
            // Configure physics properties for movable box (no gravity)
            body.setBounce(0, 0);  // No bouncing
            body.setDrag(800, 800);  // Moderate drag for gradual stop when not pushed
            body.setFriction(0.5, 0.5);  // Moderate friction
            body.setMass(3); // Lighter mass for more responsive pushing
            body.setMaxVelocity(200, 200); // Max speed in both directions
